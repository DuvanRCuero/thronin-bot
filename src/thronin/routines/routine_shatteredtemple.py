...
class ShatteredTempleRoutine(Routine):
    def __init__(self):
        super().__init__("shattered_temple")
        self._required_trackers = []
        self.pretty_name = "Shattered Temple"
        self.search_name = "purelight hill"  # using purelight tp here
        self.map_tp_coords = {
            720: [808, 473],
            1080: [1256, 741],
        }
    ...
    def _get_positioned(self):
        ...
        kbm.use_keyboard(Key.right, press_time=0.2, post_time=0.25)
        kbm.run_and_jump(11)
        kbm.use_keyboard(options.get_keybind_key("keybind_move_backward"), press_time=4)


shattered_temple = ShatteredTempleRoutine()
