...
class AbandonedStonemasonTownRoutine(Routine):
    def __init__(self):
        super().__init__("abandoned_stonemason_town")
        self._required_trackers = []
        self.pretty_name = "Abandoned Stonemason Town"
        self.search_name = "abandoned"
        self.map_tp_coords = {
            720: [560, 430],
            1080: [820, 663],
        }
    ...
    def _get_positioned(self):
        ...
        kbm.use_keyboard(Key.left, press_time=0.2, post_time=0.25)
        kbm.run_and_jump(10)
        kbm.use_keyboard(options.get_keybind_key("keybind_move_backward"), press_time=1)


abandoned_stonemason_town = AbandonedStonemasonTownRoutine()
